...
/* Local: upstream uses cos_fast/sin_fast/tan_fast without definitions
 * (intended CMSIS-DSP replacements). Map to libm for portable GCC builds. */
...
void resetState()
{
    ...
    /* Local: removed upstream for-loop writing 18 elems into restLastGyrLp[3]
     * (OOB). C++ VQF resetState only fills motionBiasEst* via fill below. */
    vqf_fill_double(state.motionBiasEstRLpState, 9*2, NaN);
    // std::fill(state.motionBiasEstRLpState, state.motionBiasEstRLpState + 9*2, NaN);
    vqf_fill_double(state.motionBiasEstBiasLpState, 2*2, NaN);
    // std::fill(state.motionBiasEstBiasLpState, state.motionBiasEstBiasLpState + 2*2, NaN);
...
{
    /* Local fix vs upstream dusking1/vqf-c: init_params() was never called,
     * leaving BSS-zero params (breaks filterCoeffs / bias defaults). */
    init_params();
    coeffs.gyrTs = gyrTs;
